...
def evaluate_mcdlstm(df, regression, n_splits=4):
    ...
    for i in range(0, n_splits):
        ...
        if regression:
            mse, f1 = visualise_regr_results(model, x_test[0:10], y_test[0:10], device=device, show_charts=True)
        else:
            mse, f1 = visualise_clf_results(model, x_test[0:10], y_test[0:10], device=device, show_charts=True)
        plt.grid()
        plt.title(f'Predictions for split {i}')
        plt.xlabel('Test sample')
        model_type = 'regr' if regression else 'clf'
        plt.savefig(f'predictions_{model_type}_split{i}.png')
        f1_vals.append(f1)
        mse_vals.append(mse)

    return f1_vals, mse_vals, f1_vals_train, mse_vals_train


df = load_data()
f1_vals_regr, mse_vals_regr, f1_vals_train_regr, mse_vals_train_regr = evaluate_mcdlstm(df, True)
f1_vals_clf, mse_vals_clf, f1_vals_train_clf, mse_vals_train_clf = evaluate_mcdlstm(df, False)
